...
export default async function customerProductsRoutes(fastify) {
    ...
    fastify.post("/:customerId/favorites", async (request, reply) => {
        ...
        try {
            ...
            const productObjectIds = products.map((product) => new mongoose.Types.ObjectId(product._id));
            ...
            const updatedCustomer = await Customer.findById(customerId).populate("favoriteProducts", "name price category");
            if (!updatedCustomer) {
                reply.code(404);
                return {
                    statusCode: 404,
            ...
            if (error instanceof mongoose.Error.MongooseServerSelectionError) {
                ...
                return {
                    ...
                    message: "Não foi possível conectar com o banco de dados. Tente novamente em alguns instantes.",
                    timestamp: new Date().toISOString(),
                    path: `/${customerId}/favorites`,
                };
            }
            ...
            return {
                ...
                message: "Ocorreu um erro inesperado ao processar sua requisição. Nossa equipe foi notificada.",
                timestamp: new Date().toISOString(),
                path: `/${customerId}/favorites`,
    ...
    fastify.delete("/:customerId/favorites/remove", async (request, reply) => {
        ...
        const validationResult = RemoveFavoriteProductsSchema.safeParse(request.body);
        if (!validationResult.success) {
            reply.code(400);
            return {
                statusCode: 400,
        ...
        try {
            ...
            const existingFavoriteProducts = customerExists.favoriteProducts || [];
            const updatedFavoriteProducts = existingFavoriteProducts.filter((productId) => !productIds.includes(productId.toString()));
